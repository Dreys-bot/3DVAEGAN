    ...
    for epoch in range(args.n_epochs):
        for i, (images, model_3d) in enumerate(dset_loaders):
            ...
            if g_loss is not None:
                g_loss.backward(inputs=list(G.parameters())) # Add generator parameter to avoid backward error
            G_solver.step()
        ...
        print(
            'Iter-{}; ,Recon_loss : {:.4} , KLLoss: {:.4}, D_loss : {:.4}, G_loss : {:.4}, D_acu : {:.4}, D_lr : {:.4}'.format(
                iteration, recon_loss.data.item(), kl_loss.data.item(), d_loss.data.item(), g_loss.data.item(), d_total_acu.data.item(),
